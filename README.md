...
Currently source-sets like `:neo-1.21.5 testmod1` can see source-sets like `:vanilla modTestmod1Version1_20_1` from the IDE. Compiling seems fine and they correctly aren't on each other's classpaths, though. This is clearly bad because they are for different minecraft versions... I've only isolated this as far as "adding `modSplat` to the `implementation` configuration" so maybe `modSplat` has too many artifacts in it?? Or an intellij bug? Weird
