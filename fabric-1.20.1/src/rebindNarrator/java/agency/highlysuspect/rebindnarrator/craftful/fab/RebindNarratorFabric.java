...
public class RebindNarratorFabric extends RebindNarratorMc implements AfterQuatlibClientInitializer {
	...
	public void onInitializeClient() {
		...
		if(config.get(NMUK_SUPPORT)) {
			if(FabricLoader.getInstance().isModLoaded("nmuk")) {
				LOG.info("No More Useless Keys support is enabled and 'nmuk' is present, enabling support.");
				nmukShim = new NmukSupport();
			} else {
				LOG.info("No More Useless Keys support is enabled but 'nmuk' is not present, using regular impl.");
				nmukShim = new NmukShim();
			}
		} else {
			LOG.info("No More Useless Keys support is disabled, using regular impl.");
			nmukShim = new NmukShim();
		}
	...
	public static final ConfigOpt<Boolean> NMUK_SUPPORT = new ConfigOpt.BoolOpt("NMUK support", true,
	...
	public ConfigSection visitConfigSchema(ConfigSection root) {
		...
		root.add(CTRL);
		root.subsection("Compat").add(AMECS_SUPPORT, NMUK_SUPPORT);
		return root;
	}
	...
	public static RebindNarratorFabric inst() {
		return (RebindNarratorFabric) RebindNarratorBase.INST;
	}
}
