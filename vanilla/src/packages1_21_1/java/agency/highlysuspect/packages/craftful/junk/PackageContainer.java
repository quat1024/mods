...
public class PackageContainer implements Container {
	...
	public boolean allowedInPackageAtAll(ItemStack stack) {
		return PackageRules.DEFAULT.allowedInPackageAtAll(stack);
	}
	...
	public static PackageContainer fromPackageContents(@Nullable ImmutablePackageContents imm) {
		PackageContainer cont = new PackageContainer();
		cont.mutateFromPackageContents(imm);
		return cont;
	}
	...
	public void mutateFromPackageContents(@Nullable ImmutablePackageContents imm) {
		...
		if(imm == null || imm.isEmpty()) return;
		
		//spread the count across the eight slots of the package
		ItemStack stack = imm.stack();
		int maxPerSlot = maxStackAmountAllowed(stack);
		for(int remaining = imm.count(), slot = 0; remaining > 0 && slot < SLOT_COUNT; remaining -= maxPerSlot, slot++) {
	...
	public ImmutablePackageContents toPackageContents() {
		if(isEmpty()) return ImmutablePackageContents.EMPTY;
		else return new ImmutablePackageContents(getFilterStack(), getCount());
	}
	...
	public static <T> T mutateItemStack(ItemStack stack, Function<PackageContainer, T> action, T ifNoContainer) {
		ImmutablePackageContents contents = stack.get(ImmutablePackageContents.DATA_COMPONENT_TYPE);
		if(contents == null) return ifNoContainer;
		
		PackageContainer cont = fromPackageContents(contents);
		try {
			return action.apply(cont);
		} finally {
			stack.set(ImmutablePackageContents.DATA_COMPONENT_TYPE, cont.toPackageContents());
		}
	}
}
