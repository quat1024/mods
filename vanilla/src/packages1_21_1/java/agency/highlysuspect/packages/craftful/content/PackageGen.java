...
public class PackageGen implements PGen {
	...
	public Block constructBlock() {
		return new PackageBlock(BlockBehaviour.Properties.ofLegacyCopy(Blocks.OAK_PLANKS).strength(1f, 1f).noOcclusion().pushReaction(PushReaction.DESTROY));
	}
	
	protected Item constructItem() {
		return new PackageItem(PLatches.Blocks.PACKAGE.get(), new Item.Properties()
			.component(PackageStyle.DATA_COMPONENT_TYPE, PackageStyle.ERROR_LOL)
			.component(ImmutablePackageContents.DATA_COMPONENT_TYPE, ImmutablePackageContents.EMPTY)
		);
	}
